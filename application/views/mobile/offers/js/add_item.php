...
if( array_key_exists( "offers_type", $page_content) ):
    $offers_type    = $page_content["offers_type"];
else:
    $offers_type    = 'buy';
endif;
...
<script>
    ...
    $('.advpost__add-btn').fancybox({
        ...
        'beforeShow' : function () {
            ...
            $('#advpost__theme-name').val( 'none' ).change().addClass('is-placeholder');
            $('#advpos__brand').val( 'none' ).change().addClass('is-placeholder');
            ...
            $('input[name=offers__barter_text]').val('');
    ...
    $('#input__barter').change( function () {
        if( $(this).prop('checked') == true ) {
            ...
            $('input[name=offers__barter_text]').focus();
        }
        else {
    ...
    $('.ajax__offer_add').click( function ( event ) {
        ...
        var action          = $('#ajax-input-action').val(),
            ...
            title           = $('input[name="offers__title"]').val(),
            keywords        = $('input[name="offers__keywords"]').val(),
            content         = $('textarea[name="offers__content"]').val(),
            price           = $('input[name="offers__price"]').val(),
            max_price       = $('input[name="offers__max_price"]').val(),
            barter          = false,
            barter_text     = $('input[name="offers__barter_text"]').val(),
            images          = [],
            existing_images = [];
        ...
        if( category == 'none' || title.length == 0 ) {
            ...
            if( title.length == 0  )
                $('input[name="offers__title"]').addClass('input__wrong_data');
                ...
                $.ajax({
                    ...
                    success: function(result){
                        if (result) {
                            ...
                            <?php if( $page == 'offers' ):?>
                                <?php /*
                                if( result.type != "<?php echo $page_content["ads_type"];?>" || result.is_first_offer){
                                    switch (result.type){
                                        case "buy":
                                            window.location.href = "https://exdor.ru/offers/buy?add=success";
                                            break;
                                        case "service":
                                            window.location.href = "https://exdor.ru/offers/service?add=success";
                                            break;
                                        case "sell":
                                            window.location.href = "https://exdor.ru/offers?add=success";
                                            break;
                                    }
                                } else
                                {
                    ...
                    success: function(data){
                        if (data) {
                            ...
                            if( data.type != "<?php echo $offers_type;?>"){
                                switch (data.type){
                                    case "buy":
                                        window.location.href = "https://exdor.ru/offers/buy?edit=success";
                                        break;
                                    case "service":
                                        window.location.href = "https://exdor.ru/offers/service?edit=success";
                                        break;
                                    case "sell":
                                        window.location.href = "https://exdor.ru/offers?edit=success";
                                        break;
                                }
                            } else {
                                $('.notify-trigger--success').attr('data-notifyTitle', 'Готово')
