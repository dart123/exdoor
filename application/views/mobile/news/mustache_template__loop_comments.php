...
<script type="text/html" id="mustache__news_comments">
    ...
    <div class="reply news__comment-{{comment_id}}">
        ...
        <div class="reply__control">
            {{#is_author}}
                {{#editable}}
                    <a title="Редактировать" class="ajax-edit-message pointer is-blue-text" data-message-id="{{comment_id}}" onclick=""><i class="fa fa-pencil"></i></a> |
                {{/editable}}
            {{/is_author}}
            {{#removable}}
                <a title="Удалить" class="ajax-remove-message pointer is-red-text" data-message-id="{{comment_id}}" onclick=""><i class="fa fa-trash-o"></i></a>
            {{/removable}}
        </div>
    </div>
    {{/.}}
