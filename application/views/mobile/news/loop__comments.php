...
<div class="reply news__comment-<?php echo $comment->id;?>">
    ...
    <div class="reply__date">
        <?php echo $comment->m_date;?>
    </div>
    <div class="reply__control">
        <?php if ( $comment->is_author ):?>
            <?php if( $comment->editable):?>
                <a title="Редактировать" class="ajax-edit-message pointer is-blue-text" data-message-id="<?php echo $comment->id;?>" onclick=""><i class="fa fa-pencil"></i></a> |
            <?php endif;?>
        <?php endif;?>
        <?php if( $comment->removable ):?>
            <a title="Удалить" class="ajax-remove-message pointer is-red-text" data-message-id="<?php echo $comment->id;?>" onclick=""><i class="fa fa-trash-o"></i></a>
        <?php endif;?>
    </div>
</div>
