...
<script type="text/html" id="mustache__message_item">
    ...
    <tr class="conversation__row message-id-{{id}}">
        ...
        <td class="conversation__action">
            ...
            <a title="Удалить" class="ajax-remove-message is-red-text" data-message-id="{{id}}" data-chatroom-id="<?php echo $page_content["chatroom"];?>" onclick=""><i class="fa fa-trash-alt"></i></a>

            {{#is_author}}
                {{#editable}}
                    <a title="Редактировать" class="ajax-edit-message is-blue-text" data-message-id="{{id}}" data-chatroom-id="<?php echo $page_content["chatroom"];?>" onclick=""><i class="fa fa-pen"></i></a>
                {{/editable}}
            {{/is_author}}
