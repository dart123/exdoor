    ...
    <div class="news-advpost item-news-{{id}}" id="js__news_list__news_{{id}}">
        ...
        <div class="news-advpost__block is-mbtm-30 is-rounded is-box-shadow">
            <div class="news-post__wrapper">
                ...
                <a href="#news-post{{id}}" data-fancybox="news-group" data-id="{{id}}" class="lower-layer fancybox"></a>
                <!--    -->
                            ...
                            <div class="frame js-inner-page-slider-w" data-slider-id="{{id}}">
                                <ul class="clearfix">
                                    {{#images}}
                                        {{#.}}
                                            <li>
                                                <a href="/uploads/news/{{id}}/lg1000_{{.}}" class="fancybox-thumb-w" data-fancybox="fancybox-thumb-{{id}}-m">
                                                    <img src="/uploads/news/{{id}}/medium_{{.}}" class="img-responsive" >
                                                </a>
                                            </li>
                                        {{/.}}
            ...
            <div class="news-post__sub">
                <div class="news-advpost__feedback is-grey-text">
                    <div class="feedback__comments" data-news-id="{{id}}" data-user-id="<?php echo $user->id;?>" data-is-liked="{{#liked}}1{{/liked}}{{^liked}}0{{/liked}}">
                        {{#comments_count}}
                        <i class="fas fa-comment"></i> <span>{{comments_count}} {{comments_count_text}}</span> {{#if_comments_count}}<a class="js-show-all-comments show-all-comments" data-news-id="{{id}}">(показать все)</a>{{/if_comments_count}}
                        {{/comments_count}}
                        {{^comments_count}}
                    ...
                    {{#comments}}
                        <div class="reply news__comment-{{id}}">
                            ...
                            <div class="reply__date">
                                {{#is_author}}
                                    {{#editable}}
                                        <a title="Редактировать" class="ajax-edit-message is-blue-text" data-message-id="{{id}}" onclick=""><i class="fas fa-pen"></i></a> |
                                    {{/editable}}
                                {{/is_author}}
                                {{#removable}}
                                    <a title="Удалить" class="ajax-remove-message is-red-text" data-message-id="{{id}}" onclick=""><i class="fas fa-trash-alt"></i></a> |
                                {{/removable}}
                                {{date}}
                            </div>
                        </div>
