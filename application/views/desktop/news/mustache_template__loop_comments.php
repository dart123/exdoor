...
<script type="text/html" id="mustache__news_comments">
    ...
    <div class="reply news__comment-{{comment_id}}">
        ...
        <div class="reply__date">
            {{#is_author}}
                {{#editable}}
                    <a title="Редактировать" class="ajax-edit-message pointer is-blue-text" data-message-id="{{comment_id}}" onclick=""><i class="fas fa-pen"></i></a> |
                {{/editable}}
            {{/is_author}}
            {{#removable}}
                <a title="Удалить" class="ajax-remove-message pointer is-red-text" data-message-id="{{comment_id}}" onclick=""><i class="fas fa-trash-alt"></i></a> |
            {{/removable}}
            {{date}}
        </div>
    </div>
