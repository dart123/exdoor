                ...
                <div class="is-mtop-20">
                    ...
                    <div class="requests-info__block is-rounded is-box-shadow">
                        <div class="requests-info__photo">
                            <input id="eq__id" type="hidden" value="<?php echo $request_data->eq_id;?>">
                        ...
                        <div class="requests-info__content">
                                    ...
                                    <?php foreach ($equipment->images as $key => $img):?>
                                        <?php if ( $key == 1 ):?>
                                            <span class="requests-descr">
                                                <a class="js__open_images_library pointer is-blue-link" data-can-select="false">
                            ...
                            <?php else:?>
                                ...
                                <p>
                                    <span class="requests-descr">
                                        <a class="js__open_images_library is-blue-link pointer" data-can-select="false">
                ...
                <div class="is-mtop-30">
                    ...
                    <div class="requests-step__block is-rounded is-box-shadow">
                        <form action="/requests/add" method="POST" class="request__add_form">
                            ...
                            <?php if(!empty($request_positions)):?>

                                <?php foreach ($request_positions as $key => $position):?>
                                    <!-- Первая позиция -->
                                    <div class="requests-step__form-2">
                                        ...
                                        <div class="req__col">
                                            <input type="text" name="detail[]" class="requests-col__input pos_val_d" id="" placeholder="Название детали" value="<?php echo $position->detail;?>">
                                            <!--  -->
                                            <div class="req__col__3_4">
                                                <input type="text" name="catalog_num[]" class="requests-col__input pos_val_cn" id="" placeholder="Номер в каталогах" value="<?php echo $position->catalog_num;?>">
                                            </div>
                                            <div class="req__col__1_4">
                                                <input type="text" name="amount[]" class="" id="" placeholder="Количество" value="<?php echo $position->amount;?>">
                                            </div>
                                                ...
                                                <?php if($position->images_arr):?>
                                                    <?php foreach ( $position->images_arr as $img):?>
                                                        <li class="request_edit__position__existing">
                                                            <img src="/uploads/equipment/<?php echo $request_data->eq_id;?>/158x158_<?php echo $img;?>"  data-original-src="<?php echo $img;?>"/>
                                                            <a class="remove js__remove_from_position" data-image-original-name="<?php echo $img;?>"></a>
                                                            <a class="edit js__image_edit__open_editor" data-image-original-url="/uploads/equipment/<?php echo $request_data->eq_id;?>/lg1000_<?php echo $img;?>" data-image-original-name="<?php echo $img;?>"></a>
                                                        </li>
                                                    <?php endforeach;?>
                                                <?php endif;?>
                                            </ul>
                            ...
                            <?php else:?>
                                ...
                                <div class="requests-step__form-2">
                                    ...
                                    <div class="req__col">
                                        <input type="text" name="detail[]" class="requests-col__input pos_val_d" id="" placeholder="Название детали">
                                        <!--  -->

                                        <div class="req__col__3_4">
                                            <input type="text" name="catalog_num[]" class="requests-col__input pos_val_cn" id="" placeholder="Номер в каталогах">
                                        </div>
                                        <div class="req__col__1_4">
                                            <input type="text" name="amount[]" class="" placeholder="Количество">
                                        </div>
                            ...
                            <a class="js__request_add_positions pointer requests__next-step is-last-item btn-primary2 btn ripple-effect">Продолжить</a>
                        </form>
                    </div>
                    <!--  end Блок с формой  -->
                </div>
...
<script>
    $(document).ready( function () {
        ...
        $('#counter-form-fill__positions').roundSlider({
            ...
            radius: 50,
            width: 2,
            value: <?php echo $request_data->progress;?>,
            editableTooltip: false,
            tooltipFormat: "tooltipVal2"
        });
        $(".pos_val_d, .pos_val_cn").change(function() {

            var value       = <?php echo $request_data->progress;?>,
                details     = false,
                cat_num     = false;
                                        ...
                                        success: function(result){
                                            if (result) {
                                                ...
                                                result.images.forEach(function(item, i, data) {
                                                    ...
                                                    $(li).append("<a class='remove js__remove_from_library' data-image-original-name='" + item + "'></a>");
                                                    $(li).append("<a class='edit js__image_edit__open_editor' data-image-original-url='https://exdor.ru/uploads/equipment/<?php if ($request_data->eq_id) echo $request_data->eq_id;?>/lg1000_" + item + "' data-image-original-name='" + item + "' ></a>");
    ...
    $(document).ready( function () {
        $('.js__requests_library__user_selected').click( function () {
            ...
            selected.forEach( function(item, i, data) {
                ...
                $(li).append("<img src='/uploads/equipment/<?php if($request_data->eq_id) echo $request_data->eq_id;?>/158x158_" + item + "?v=" + Math.floor(Date.now() / 1000) + "'   data-original-src='" + item + "'/>");
                $(li).append("<a class='remove js__remove_from_position' data-image-original-name='" + item + "'></a>");
                $(li).append("<a class='edit js__image_edit__open_editor' data-image-original-url='<?=$this->config->item('base_url');?>/uploads/equipment/<?php if ($request_data->eq_id) echo $request_data->eq_id;?>/lg1000_" + item + "' data-image-original-name='" + item + "' ></a>");
            });
        ...
        $('body').on('click', '.js__remove_from_library', function() {
            ...
            $.ajax({
                url:   '/ajax/upload_to_library',
                data: {
                    'equipment_id'      : <?php echo $request_data->eq_id;?>,
                    'existing_images'   : existing_images
                },
                type: 'POST',
                dataType: 'json',
...
<?php
    ...
    $this->load->view('desktop/requests/js/__pixie_editor');

    $this->load->view('desktop/misc/js/pixie2');

    $this->load->view('desktop/requests/js/add_start_again');
    $this->load->view('desktop/requests/js/add_positions');
?>