...
<script>
    ...
    $(document).ready( function () {
        ...
        socket.on('notification', function (result) {
            ...
            <?php if( isset( $notifications__no_messages ) && $notifications__no_messages == true ):?>
                if( data.tg == 'new_message' )
                    return;
            <?php endif;?>

            <?php if( isset( $chatroom ) ):?>
                if( data.tg == 'new_message' && data.from_id == "<?php echo $opponent->id;?>" )
                    return;
            <?php endif;?>
            ...
            var sound = new buzz.sound("/assets/sfx/just-like-magic-3", {
                formats: [ "ogg", "mp3" ],
                preload: true,
                autoplay: true,
                loop: false
        ...
        <?php if(isset($chatroom)):?>
            socket.on('message', function (result) {
                ...
                if( data.chatroom_id != <?php echo $chatroom;?>) {
                    return;
                }
        ...
        socket.on('new_news', function (result) {
            ...
            if( data.is_first_news && "<?php echo $menu['selected'];?>" == "main" ) {
        ...
        socket.on('new_ads', function (result) {
            ...
            if( ( data.is_first_offer && "<?php echo $menu['selected'];?>" == "main" ) || data.type != "<?php echo isset($ads_type)? $ads_type : "none" ;?>" ) {
        ...
        socket.on('requests__list__update', function (result) {
            ...
            if ( $('.ajax__request-item_list__'+ data.user+'__' + data.request_id ).length > 0 ) {
                ...
                $('.ajax__request-item_list__'+ data.user+'__' + data.request_id ).replaceWith(output).promise().done(function(elem) {

                    $('.ajax__request-item_list__'+ data.user+'__' + data.request_id ).animate({
        ...
        socket.on('informer__company_employers__count', function (result) {
            ...
            if( parseInt( data ) > 0 ) {
                ...
                $('#js-left-sidebar-menu__employers').find(".msg-counter").remove();
            }
